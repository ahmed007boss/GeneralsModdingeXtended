...
This documentation page is currently under development. The GameData INI file contains core game data configuration settings and parameters for fundamental game systems.
...
- [Object](Object.md) - Game data affects all objects
- [Weapon](Weapon.md) - Game data affects weapons
- [SpecialPower](SpecialPower.md) - Game data affects special powers

---

**Note**: This page will be completed as part of the GMX documentation effort. For now, refer to the existing game files and source code for GameData template information.

## Example Structure

```ini
GameData
  ShellMapName = mainmenubackdropuserinterface.tga
  MapName = Assault.map
  ...
  TerrainLOD = DISABLE  ;test should be handled by options screen.
  ...
  HistoricDamageLimit = 4000  ; how long to retain historical-damage for weapons that need it, in msec
  ...
  InfantryLightEveningScale = 125f ; amount to scale map lighting to make infantry easier to see.
  InfantryLightNightScale = 150f ; amount to scale map lighting to make infantry easier to see.
  ...
  BuildSpeed = 1.5
  MinDistFromEdgeOfMapForBuild = 20.0  ; buildings may not be constructed this close to a map edge
  SupplyBuildBorder = 45.0  ; min dist you can put a supply center from a supply source
  ...
  MaxParticleCount = 5000        ; the maximum number of particles that can exist at one time (to 5000 for demo)
  MaxFieldParticleCount = 100     ; the number of field type particles rendered to the screen, beyond which this type will begin skipping
...
;"PRE_ATTACK",
  ...
  WeaponBonus =       HORDE  RATE_OF_FIRE  125%  ;Horded troops fire their weapons 25% faster
  ...
  WeaponBonus = SUBLIMINAL   RATE_OF_FIRE  110%  ;In range of an upgraded speaker tower, gaining just little enthusiasm
  WeaponBonus =  GARRISONED         RANGE  110%  ; when garrisoned, all weapons get n times normal range
  WeaponBonus =  GARRISONED        DAMAGE  130%  ; when garrisoned, all weapons get n times normal range
  WeaponBonus =  GARRISONED        RATE_OF_FIRE  100%  ; when garrisoned, all weapons get n times normal range
  ...
  WeaponBonus = DEMORALIZED_OBSOLETE RATE_OF_FIRE 50%
  WeaponBonus = DEMORALIZED_OBSOLETE DAMAGE 50%
  WeaponBonus = DEMORALIZED_OBSOLETE RANGE 50%

  WeaponBonus = FRENZY_ONE    RADIUS  300% ; Granted by a China Generals power
  WeaponBonus = FRENZY_TWO    RATE_OF_FIRE 120% ; Used for GLA fanatism
  WeaponBonus = FRENZY_TWO    RADIUS 150% ; Used for GLA fanatism
  WeaponBonus = FRENZY_TWO    DAMAGE 110% ; Used for GLA fanatism
  WeaponBonus = FRENZY_THREE  DAMAGE 130%
    
  WeaponBonus = VETERAN  RATE_OF_FIRE  110%  
  WeaponBonus = VETERAN  DAMAGE        110%
  WeaponBonus = ELITE    RATE_OF_FIRE  120%
  WeaponBonus = ELITE    DAMAGE        120%
  WeaponBonus = HERO     RATE_OF_FIRE  130%  
  WeaponBonus = HERO     DAMAGE        130%  

  WeaponBonus = DRONE_SPOTTING RANGE        420%  
...
;  VolumeMuValue = 6.0                  ; defines the steepness of the S, must be >=0, higher is steeper.
  ...
  SellPercentage       = 10% ; You get this percentage back of the cost to build
...
;  PrisonBountyTextColor = R:255 G:128 B:64 A:255
  ...
  NetworkLatencyHistoryLength = 300 ; The number of ping history entries.
  NetworkRunAheadMetricsTime = 500 ; The number of miliseconds between run ahead metrics things
  NetworkCushionHistoryLength = 1  ; The number of cushion values to keep.
  NetworkRunAheadSlack = 5         ; The amount of slack in the run ahead value.  This is the percentage of the calculated run ahead that is added.
  NetworkKeepAliveDelay = 30        ; The number of seconds between when the connections to each player send a keep-alive packet.
  ...
  NetworkPlayerTimeoutTime = 120000  ; The number of milliseconds between when a player's last keep alive command was recieved and when they are considered disconnected from the game.
...
- Header: [`GeneralsMD/Code/GameEngine/Include/GameLogic/GameData.h`](../GeneralsMD/Code/GameEngine/Include/GameLogic/GameData.h)
- Source: [`GeneralsMD/Code/GameEngine/Source/GameLogic/GameData/GameData.cpp`](../GeneralsMD/Code/GameEngine/Source/GameLogic/GameData/GameData.cpp)
