...
## Overview
`CommunicationComponent` models communication systems (links, radios) under `[ActiveBody](../ObjectModules/ActiveBody.md)`.

Available in: *(GMX)*

## Usage
Place under `Body = ActiveBody` to make communication systems damageable, healable, replaceable, and usable in various gameplay systems.
...
- If the name token is omitted/empty, a default name is auto-assigned for this type (e.g., `CommunicationRadio`).
...
#### `MaxHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Maximum health points for this communication component. Higher values make the component more durable and resistant to damage. This determines the total damage capacity before the component is destroyed
- **Default**: `0.0`
- **Example**: `MaxHealth = 50.0`

#### `InitialHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Starting health points for this communication component when the object is created. Higher values allow components to spawn with more health than their maximum, providing temporary damage buffer
- **Default**: `0.0`
- **Example**: `InitialHealth = 50.0`

### Healing Settings *(GMX)*

#### `HealingType` *(GMX)*
- **Type**: `ComponentHealingType` (see [ComponentHealingType Values](#componenthealingtype-values) section)
- **Description**: Defines how this communication component can be healed. Different healing types control whether components can be fully repaired, partially repaired, or require replacement
- **Default**: `NORMAL`
...
- **Description**: Specifies which hit sides can damage this communication component. If not set (empty), the component can be damaged from any hit side. This allows for realistic damage modeling where certain components are only vulnerable from specific angles
...
- **Description**: Damage state the main object enters when this communication component is damaged (between 10% and 50% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
- **Type**: `BodyDamageType` (see [BodyDamageType Values](#bodydamagetype-values) section)
- **Description**: Damage state the main object enters when this communication component is destroyed (0% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
#### `ReplacementCost` *(GMX)*
- **Type**: `UnsignedInt`
- **Description**: Cost in money to fully replace this communication component when it's damaged. Higher values make component replacement more expensive. At 0 (default), the component cannot be replaced via the GUI command system
- **Default**: `0`
- **Example**: `ReplacementCost = 150`

#### `ForceReturnOnDestroy` *(GMX)*
- **Type**: `Bool`
- **Description**: Whether the unit should be forced to return to base when this communication component is destroyed. When `Yes`, aircraft and vehicles will automatically return when this component is destroyed. When `No`, component destruction does not trigger return behavior
- **Default**: `No`
...
```ini
CommunicationComponent TacticalRadio
  MaxHealth = 35.0
  InitialHealth = 35.0
  HealingType = NORMAL
...
```ini
CommunicationComponent BackupComm
  MaxHealth = 25.0
  InitialHealth = 25.0
...
- Header: `GeneralsMD/Code/GameEngine/Include/GameLogic/Components/CommunicationComponent.h`
- Source: `GeneralsMD/Code/GameEngine/Source/GameLogic/Components/CommunicationComponent.cpp`

## Changes History

- 15/12/2025 — AI — Initial documentation with full property details and enum values.
- 15/12/2025 — AI — Updated Limitations/Conditions/Dependencies to reflect weapon and locomotor interactions.
- 15/12/2025 — AI — Added internal/external links and corrected command constant name.
