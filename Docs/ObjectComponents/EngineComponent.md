...
## Overview
`EngineComponent` models a ground vehicle engine under `[ActiveBody](../ObjectModules/ActiveBody.md)`. When damaged/destroyed, mobility may be impaired depending on game logic.

Available in: *(GMX)*

## Usage
Place under `Body = ActiveBody` to make engine systems damageable, healable, replaceable, and usable in movement systems.
...
- If the name token is omitted/empty, a default name is auto-assigned for this type (e.g., `MainEngine`).
...
- For `LocomotorTemplate` to use this engine component, the locomotor must reference this component by name via `EngineComponentName` (see Locomotor documentation). If not referenced, the locomotor will not check engine component status.
...
- If this component's name is set as `LocomotorTemplate.EngineComponentName`, the locomotor will check this component's status to determine if the engine is functional (see Locomotor documentation).
...
#### `MaxHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Maximum health points for this engine component. Higher values make the component more durable and resistant to damage. This determines the total damage capacity before the component is destroyed
- **Default**: `0.0`
- **Example**: `MaxHealth = 150.0`

#### `InitialHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Starting health points for this engine component when the object is created. Higher values allow components to spawn with more health than their maximum, providing temporary damage buffer
- **Default**: `0.0`
- **Example**: `InitialHealth = 150.0`

### Healing Settings *(GMX)*

#### `HealingType` *(GMX)*
- **Type**: `ComponentHealingType` (see [ComponentHealingType Values](#componenthealingtype-values) section)
- **Description**: Defines how this engine component can be healed. Different healing types control whether components can be fully repaired, partially repaired, or require replacement
- **Default**: `NORMAL`
...
- **Description**: Specifies which hit sides can damage this engine component. If not set (empty), the component can be damaged from any hit side. This allows for realistic damage modeling where certain components are only vulnerable from specific angles
- **Default**: Empty (damageable from all sides)
- **Example**: `DamageOnSides = HIT_SIDE_BACK HIT_SIDE_LEFT HIT_SIDE_RIGHT`
...
- **Description**: Damage state the main object enters when this engine component is damaged (between 10% and 50% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
- **Type**: `BodyDamageType` (see [BodyDamageType Values](#bodydamagetype-values) section)
- **Description**: Damage state the main object enters when this engine component is destroyed (0% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
#### `ReplacementCost` *(GMX)*
- **Type**: `UnsignedInt`
- **Description**: Cost in money to fully replace this engine component when it's damaged. Higher values make component replacement more expensive. At 0 (default), the component cannot be replaced via the GUI command system
- **Default**: `0`
- **Example**: `ReplacementCost = 500`

#### `ForceReturnOnDestroy` *(GMX)*
- **Type**: `Bool`
- **Description**: Whether the unit should be forced to return to base when this engine component is destroyed. When `Yes`, vehicles will automatically return when this component is destroyed. When `No`, component destruction does not trigger return behavior
- **Default**: `No`
...
- Header: `GeneralsMD/Code/GameEngine/Include/GameLogic/Components/EngineComponent.h`
- Source: `GeneralsMD/Code/GameEngine/Source/GameLogic/Components/EngineComponent.cpp`
