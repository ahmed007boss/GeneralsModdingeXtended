# <span style="color:#5bbcff; font-weight:bold;">VisionComponent</span> *(GMX)*

Status: AI-generated, 0/2 reviews

## Overview
VisionComponent models optics and vision hardware attached via body modules. It provides shroud clearing capabilities based on component health status. VisionComponent inherits all properties from [Component](Component.md) and adds vision/shroud clearing properties. This is a module added inside `Object` entries.
...
- **Description**: Which hit sides can damage this vision component. If empty, all sides can damage the component. If specified, only listed sides can cause damage. Used for side-specific armor on components. Parsed using full hit side names with "HIT_SIDE_" prefix: `HIT_SIDE_FRONT`, `HIT_SIDE_BACK`, `HIT_SIDE_LEFT`, `HIT_SIDE_RIGHT`, `HIT_SIDE_TOP`, `HIT_SIDE_BOTTOM`. Note: `HIT_SIDE_UNKNOWN` exists in the enum but is not parseable for this property.
...
- **Type**: `UnsignedInt`
- **Description**: Money cost to fully replace this vision component via GUI command system. At 0 (default), the component cannot be replaced via GUI commands. Replacement fully restores component health to maximum.
- **Default**: `0`
...
- **Type**: `Bool`
- **Description**: Whether the unit should be forced to return to base when this vision component is destroyed. When `Yes`, aircraft will automatically return to base when this component is destroyed (checked by [JetAIUpdate](../ObjectUpdates/JetAIUpdate.md)). When `No`, component destruction does not trigger return behavior.
- **Default**: `No`
...
- **Type**: `BodyDamageType` (see [BodyDamageType Values](#bodydamagetype-values))
- **Description**: Body damage state to set when the vision component is destroyed (0% - 10% health, which corresponds to downed status). Controls visual appearance of the object model. Set to one of the [BodyDamageType Values](#bodydamagetype-values) or leave empty/unset (0) for no visual change. The damage state is applied to the main object's body when component health falls within this range.
- **Default**: `0` (no change)
...
- **Description**: Animation icon template name displayed in GUI when vision component is partially functional (10% - 50% health). References an animation template defined in [Animation2d](../Animation2d.md) configuration.
- **Default**: Empty (no icon)
- **Example**: `PartiallyFunctionalIcon = VisionDamagedIcon`
...
- **Type**: `AsciiString` (see [Animation2d](../Animation2d.md))
- **Description**: Animation icon template name displayed in GUI when vision component is downed (0% - 10% health). References an animation template defined in [Animation2d](../Animation2d.md) configuration.
- **Default**: Empty (no icon)
- **Example**: `DownedIcon = VisionDestroyedIcon`
...
- **Type**: `AsciiString` (see [Animation2d](../Animation2d.md))
- **Description**: Animation icon template name displayed in GUI when vision component is user-disabled. References an animation template defined in [Animation2d](../Animation2d.md) configuration.
- **Default**: Empty (no icon)
- **Example**: `UserDisabledIcon = VisionDisabledIcon`
...
VisionComponent Optics
  ...
  PartiallyFunctionalShroudClearingRange = 200.0
  DisabledShroudClearingRange = 50.0
End
```

```ini
VisionComponent MainVision
  MaxHealth = 50%
  InitialHealth = 50%
  HealingType = NORMAL
  ReplacementCost = 250
  ForceReturnOnDestroy = Yes
  PartiallyFunctionalIcon = VisionDamagedIcon
  DownedIcon = VisionDestroyedIcon
...
```ini
VisionComponent LongRangeSight
  MaxHealth = 25.0
  InitialHealth = 25.0
  HealingType = NORMAL
  ...
  PartiallyFunctionalIcon = VisionDamagedIcon
  DownedIcon = VisionDestroyedIcon
  UserDisabledIcon = VisionDisabledIcon
  ShroudClearingRange = 500.0
  PartiallyFunctionalShroudClearingRange = 350.0
  DisabledShroudClearingRange = 150.0
End
```

## Usage
Place under a body module (e.g., `Body = ActiveBody`, `Body = StructureBody`) to make vision systems damageable, healable, replaceable, and provide vision/shroud clearing capabilities. See Template for correct syntax.

Multiple component instances can be added to the same body. Each component operates independently with its own health tracking and vision range. Component names must be unique within the same body.
...
- Name is optional for this type. If the name token is omitted/empty, a default name of `Vision` is automatically assigned. If a name is explicitly provided, it must be unique within the same body.
...
**Note**: If `NAME` is omitted, the component is automatically assigned the default name `Vision`.

## Notes
- Vision components provide shroud clearing capabilities based on component health status.
- The effective shroud clearing range depends on component status. When multiple vision components exist on the same object, the object's effective shroud clearing range is the maximum of all vision components' effective ranges.
...
- If the name token is omitted, the component is automatically assigned the default name `Vision`. This makes it easy to add a default vision component without specifying a name.
...
- Header: [VisionComponent.h](../../GeneralsMD/Code/GameEngine/Include/GameLogic/Components/VisionComponent.h)
- Source: [VisionComponent.cpp](../../GeneralsMD/Code/GameEngine/Source/GameLogic/Components/VisionComponent.cpp)

## Changes History

- GMX Zero Hour — Adds VisionComponent (new GMX-only component type for shroud clearing/vision effects).

## Document Log

- 16/12/2025 — AI — Initial document created and fully reconstructed per instruction guide (inherited properties, enums, vision properties, placement, links, defaults).
