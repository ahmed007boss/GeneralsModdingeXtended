...
## Overview
`JetEngineComponent` models a jet engine attached via `[ActiveBody](../ObjectModules/ActiveBody.md)`. When destroyed, aircraft typically return to base if configured.

Availability: *(GMX)*

## Usage
Place under `Body = ActiveBody` to make jet engine systems damageable, healable, replaceable, and usable in aircraft movement systems.
...
- If the name token is omitted/empty, a default name is auto-assigned for this type (e.g., `JetEngine`).
...
#### `MaxHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Maximum health points for this jet engine component. Higher values make the component more durable and resistant to damage. This determines the total damage capacity before the component is destroyed
- **Default**: `0.0`
- **Example**: `MaxHealth = 100.0`

#### `InitialHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Starting health points for this jet engine component when the object is created. Higher values allow components to spawn with more health than their maximum, providing temporary damage buffer
- **Default**: `0.0`
...
- **Description**: Defines how this jet engine component can be healed. Different healing types control whether components can be fully repaired, partially repaired, or require replacement
- **Default**: `NORMAL`
...
- **Description**: Specifies which hit sides can damage this jet engine component. If not set (empty), the component can be damaged from any hit side. This allows for realistic damage modeling where certain components are only vulnerable from specific angles
- **Default**: Empty (damageable from all sides)
- **Example**: `DamageOnSides = HIT_SIDE_BACK`
...
- **Description**: Damage state the main object enters when this jet engine component is damaged (between 10% and 50% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
- **Type**: `BodyDamageType` (see [BodyDamageType Values](#bodydamagetype-values) section)
- **Description**: Damage state the main object enters when this jet engine component is destroyed (0% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
#### `ReplacementCost` *(GMX)*
- **Type**: `UnsignedInt`
- **Description**: Cost in money to fully replace this jet engine component when it's damaged. Higher values make component replacement more expensive. At 0 (default), the component cannot be replaced via the GUI command system
- **Default**: `0`
- **Example**: `ReplacementCost = 400`

#### `ForceReturnOnDestroy` *(GMX)*
- **Type**: `Bool`
- **Description**: Whether the unit should be forced to return to base when this jet engine component is destroyed. When `Yes`, aircraft will automatically return to base when this component is destroyed. When `No`, component destruction does not trigger return behavior. Typically set to `Yes` for jet engines
- **Default**: `No`
...
## Notes
- Use multiple jet engine components for multi-engine aircraft to model partial failures.
- When `ForceReturnOnDestroy = Yes`, aircraft will automatically return to base if the engine is destroyed.

## Source Files

**Base Class:** [Component](../../GeneralsMD/Code/GameEngine/Include/GameLogic/Components/Component.h)

- Header: `GeneralsMD/Code/GameEngine/Include/GameLogic/Components/JetEngineComponent.h`
- Source: `GeneralsMD/Code/GameEngine/Source/GameLogic/Components/JetEngineComponent.cpp`
