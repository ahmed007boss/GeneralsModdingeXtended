...
/** Xfer Method
	...
	* 4: ahmed007boss 	 @tweak Support for 32 players (m_radarHidden and m_radarForceOn size increased to 32)
	*/
// ------------------------------------------------------------------------------------------------
void Radar::xfer( Xfer *xfer )
{
	...
	{
		// Version 2-3: 16 players (16 bytes)
		Bool oldRadarHidden[16];
		Bool oldRadarForceOn[16];

		if (xfer->getXferMode() == XFER_SAVE)
		{
			// Copy first 16 players to old arrays
			for (Int i = 0; i < 16; ++i)
			{
				oldRadarHidden[i] = m_radarHidden[i];
				oldRadarForceOn[i] = m_radarForceOn[i];
			}
			xfer->xferUser(&oldRadarHidden, sizeof(oldRadarHidden));
			xfer->xferUser(&oldRadarForceOn, sizeof(oldRadarForceOn));
		}
		else
		{
			// Load old format (16 players)
			xfer->xferUser(&oldRadarHidden, sizeof(oldRadarHidden));
			xfer->xferUser(&oldRadarForceOn, sizeof(oldRadarForceOn));
			
			// Copy to new arrays, zero out remaining players
			for (Int i = 0; i < 16; ++i)
			{
				m_radarHidden[i] = oldRadarHidden[i];
				m_radarForceOn[i] = oldRadarForceOn[i];
			}
			// Initialize remaining players to false
			for (Int i = 16; i < MAX_PLAYER_COUNT; ++i)
			{
				m_radarHidden[i] = false;
				m_radarForceOn[i] = false;
			}
		}
	}
	else
	{
		// Version 4+: Support for MAX_PLAYER_COUNT players
		// Note: sizeof() automatically handles the correct array size based on MAX_PLAYER_COUNT
