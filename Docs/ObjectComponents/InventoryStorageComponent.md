...
## Overview
`InventoryStorageComponent` provides a health-bearing storage (e.g., fuel tank) attached via `[ActiveBody](../ObjectModules/ActiveBody.md)`. Destruction may impact availability of stored items.

Availability: *(GMX)*

## Usage
Place under `Body = ActiveBody` to make inventory storage systems damageable, healable, replaceable, and usable in inventory management systems.
...
- Name is required for this type. If the name token is omitted/empty, the component is skipped during parsing and not added to the object.
...
- If `[ForceReturnOnDestroy](#forcereturnondestroy-gmx)` is set to `Yes`, aircraft will automatically return to base when this component is destroyed (checked by `[JetAIUpdate](../ObjectUpdates/JetAIUpdate.md)`).
...
- The `[InventoryItem](#inventoryitem-gmx)` property links this component to a specific inventory item type. When the component is destroyed, stored items may be lost depending on game logic.
...
#### `MaxHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Maximum health points for this inventory storage component. Higher values make the component more durable and resistant to damage. This determines the total damage capacity before the component is destroyed
- **Default**: `0.0`
- **Example**: `MaxHealth = 60.0`

#### `InitialHealth` *(GMX)*
- **Type**: `Real`
- **Description**: Starting health points for this inventory storage component when the object is created. Higher values allow components to spawn with more health than their maximum, providing temporary damage buffer
- **Default**: `0.0`
...
- **Description**: Defines how this inventory storage component can be healed. Different healing types control whether components can be fully repaired, partially repaired, or require replacement
- **Default**: `NORMAL`
...
- **Description**: Specifies which hit sides can damage this inventory storage component. If not set (empty), the component can be damaged from any hit side. This allows for realistic damage modeling where certain components are only vulnerable from specific angles
- **Default**: Empty (damageable from all sides)
- **Example**: `DamageOnSides = HIT_SIDE_BACK HIT_SIDE_BOTTOM`
...
- **Description**: Damage state the main object enters when this inventory storage component is damaged (between 10% and 50% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
- **Type**: `BodyDamageType` (see [BodyDamageType Values](#bodydamagetype-values) section)
- **Description**: Damage state the main object enters when this inventory storage component is destroyed (0% health). This affects visual appearance and model state
- **Default**: `0` (no change)
...
#### `ReplacementCost` *(GMX)*
- **Type**: `UnsignedInt`
- **Description**: Cost in money to fully replace this inventory storage component when it's damaged. Higher values make component replacement more expensive. At 0 (default), the component cannot be replaced via the GUI command system
- **Default**: `0`
- **Example**: `ReplacementCost = 150`

#### `ForceReturnOnDestroy` *(GMX)*
- **Type**: `Bool`
- **Description**: Whether the unit should be forced to return to base when this inventory storage component is destroyed. When `Yes`, aircraft and vehicles will automatically return when this component is destroyed. When `No`, component destruction does not trigger return behavior. Often used for fuel tanks on aircraft
- **Default**: `No`
- **Example**: `ForceReturnOnDestroy = Yes`

### Storage Settings *(GMX)*

#### `InventoryItem` *(GMX)*
- **Type**: `AsciiString`
- **Description**: Name of the inventory item type this component can store. When the component is destroyed, stored items may be lost. This links the storage component to a specific inventory item definition
- **Default**: Empty
- **Example**: `InventoryItem = JetFuel`
...
InventoryStorageComponent NAME
  ...
  InventoryItem =                 ; // name of inventory item type stored here *(GMX)*
End
```

## Notes
- Pair with engine/fuel behaviors to create meaningful gameplay when destroyed.
- Use `ForceReturnOnDestroy = Yes` for critical storage like fuel tanks on aircraft.

## Source Files

**Base Class:** [Component](../../GeneralsMD/Code/GameEngine/Include/GameLogic/Components/Component.h)

- Header: `GeneralsMD/Code/GameEngine/Include/GameLogic/Components/InventoryStorageComponent.h`
- Source: `GeneralsMD/Code/GameEngine/Source/GameLogic/Components/InventoryStorageComponent.cpp`
